...
interface WhyDifferentStepProps {
  onContinue: (value: string) => void;
}

const OPTIONS = ['committed', 'vision', 'honest', 'smallSteps', 'atlas'] as const;

export default function WhyDifferentStep({ onContinue }: WhyDifferentStepProps) {
  const { t } = useTranslation();
  const [selected, setSelected] = useState<string>('');

  useEffect(() => {
    loadOnboardingAnswer<string>('whyDifferent').then((value) => {
  ...
  const handleContinue = async () => {
    if (!selected) return;
    await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    await persistOnboardingAnswer('whyDifferent', selected);
    onContinue(selected);
  };
    ...
    <View style={localStyles.container}>
      <View style={localStyles.headerSlot}>
        <Text style={[styles.aboutYouTitle, localStyles.screenHeader]}>
          {t('onboarding.yazioFlow.whyDifferentQuestion')}
        </Text>
      </View>
          ...
          {OPTIONS.map((optionId) => {
            ...
            return (
              ...
              >
                ...
                >
                  {t(`onboarding.yazioFlow.whyDifferentOptions.${optionId}`)}
                </Text>
              </TouchableOpacity>
            );
          })}
