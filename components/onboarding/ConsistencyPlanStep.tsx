...
interface ConsistencyPlanStepProps {
  onContinue: (values: string[]) => void;
}

const OPTIONS = ['reminders', 'smallSteps', 'accountability', 'wins', 'reflection'] as const;

export default function ConsistencyPlanStep({ onContinue }: ConsistencyPlanStepProps) {
  const { t } = useTranslation();
  const [selected, setSelected] = useState('');

  useEffect(() => {
    loadOnboardingAnswer<string[] | string>('consistencyPlan').then((value) => {
  ...
  const handleContinue = async () => {
    ...
    await persistOnboardingAnswer('consistencyPlan', payload);
    onContinue(payload);
  };
