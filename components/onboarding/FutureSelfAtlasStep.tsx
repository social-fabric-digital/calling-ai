...
export default function FutureSelfAtlasStep({ onContinue }: FutureSelfAtlasStepProps) {
  ...
  const handleContinue = async () => {
    ...
    await persistOnboardingAnswer('futureSelfAtlasSeen', true);
    onContinue();
  };

  return (
    <Animated.View style={[localStyles.container, { opacity: fadeAnim, paddingTop: isNarrowScreen ? 16 : 30 }]}>
      <Text style={[localStyles.heading, isNarrowScreen && localStyles.headingNarrow]}>{t('onboarding.yazioFlow.futureSelfAtlasTitle')}</Text>
      <Text style={[localStyles.headingSubtitle, isNarrowScreen && localStyles.headingSubtitleNarrow]}>{t('onboarding.yazioFlow.futureSelfAtlasSubtitle')}</Text>
        ...
        <Animated.View style={[localStyles.speechWrap, { transform: [{ scale: bubbleAnim }] }]}>
          <View style={localStyles.thoughtBubbleWrap}>
            <View style={[localStyles.introField, { width: thoughtBubbleWidth, alignSelf: 'center' }]}>
              <Text style={[localStyles.subheading, isNarrowScreen && localStyles.subheadingNarrow]}>{t('onboarding.yazioFlow.futureSelfAtlasText')}</Text>
            </View>
