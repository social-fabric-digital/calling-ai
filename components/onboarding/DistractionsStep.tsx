...
interface DistractionsStepProps {
  onContinue: (values: string[]) => void;
}

const OPTIONS = ['social', 'work', 'thoughts', 'opinions', 'energy'] as const;

export default function DistractionsStep({ onContinue }: DistractionsStepProps) {
  const { t } = useTranslation();
  const [selected, setSelected] = useState<string[]>([]);

  useEffect(() => {
    loadOnboardingAnswer<string[]>('distractions').then((value) => {
  ...
  const handleContinue = async () => {
    ...
    await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    await persistOnboardingAnswer('distractions', selected);
    onContinue(selected);
  };
              ...
              return (
                ...
                >
                  <Text style={[styles.lifeContextOptionText, localStyles.optionText]}>
                    {t(`onboarding.yazioFlow.distractionsOptions.${optionId}`)}
                  </Text>
                </TouchableOpacity>
              );
            })}
